...
- **Custom `BottomSheet`** (`components/ui/BottomSheet.tsx`), built directly on `react-native-reanimated` + `react-native-gesture-handler` — not `@gorhom/bottom-sheet`, which is incompatible with Reanimated 4 on this stack (see §1). Generic/reusable: takes `children` and an optional `footer`, exposes an imperative `{ open, openFull, close, snapTo }` handle via `forwardRef`, opened via a ref held outside the feed's render tree so opening/closing never touches feed state.
- **Snap mechanics**: two layers for the main content, not one. An outer "viewport" (`position: absolute, bottom: 0, overflow: hidden`) is the *only* thing whose `height` is animated. Its one child is a static "card" — absolutely positioned, fixed at `fullHeight`, anchored `top: 0` so it tracks the viewport's animated top edge and reveals content top-down as the sheet grows. This split exists because a single view animating `height` while directly containing the real content (header + `FlatList` as normal flex children) forces Yoga to re-layout that whole subtree every frame — and `FlatList` bounces its `onLayout` to the JS thread on every one of those frames too. That caused real, measurable frame drops closing the sheet (see PERFORMANCE.md). Since the card's own size never changes, the animated viewport only ever has to reposition one already-sized box, not re-flow a subtree. A `Gesture.Pan()` on a small floating handle (tracking the viewport's current top edge via an animated `bottom`) drives the same shared value during drag, and `onEnd` picks the nearest of `[closed, half, full]` (velocity-weighted) and springs to it.
- **Footer (chat input) is a third, separate layer** — not a normal flex child inside the card. The card is fixed at `fullHeight`, so anything laid out inside it as a normal bottom-of-flex-column element (like an input after the message list) ends up positioned at the *card's* bottom, which is `fullHeight` below the card's top — outside the viewport's clip window at anything short of "full." That made the input invisible at "half" until this was caught and fixed: `footer` renders as its own `position: absolute` layer pinned to the sheet's true bottom edge (which never moves, since the viewport itself is always `bottom: 0`), independent of the card's clipping. `ChatMessageList`'s content gets bottom padding (`FOOTER_CLEARANCE`) so messages don't render underneath the now-floating input.
- **Backdrop**: a full-screen `Animated.View` behind the sheet, opacity driven by `sheetHeight` (fades in as the sheet opens), containing a `Pressable` that calls `snapTo("closed")`. Since it's rendered below the sheet's card in z-order, taps landing on the sheet itself are handled by the sheet, not the backdrop — only taps outside it reach the `Pressable`.
- **Keyboard**: two parts, both in `BottomSheet.tsx`. (1) A `Keyboard.addListener('keyboardWillShow' | 'keyboardDidShow', ...)` snaps the sheet to full height the moment the keyboard opens, whatever the current snap point — without this, opening the keyboard at "half" leaves less room than the header + input need. (2) `useAnimatedKeyboard()` drives a `bottom` offset on the footer layer equal to the keyboard's current height (UI-thread, synced with the native keyboard animation curve). `KeyboardAvoidingView` was tried first but dropped — its self-measurement (it measures its own position relative to the window to compute padding) is unreliable nested inside a `position: absolute` view whose height is itself being animated, and it never produced correct padding in testing.
